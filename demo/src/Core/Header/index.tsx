 ...
 */
export const Header = (p: Props): JSX.Element | null => {
  const { id, navigation, route } = p;
  ...
  const [elements, setElements] = useState<Record<string, Element | undefined>>(
    {
      [route.key]: child,
    },
  );
  useEffect(() => {
    setElements({ ...elements, [route.key]: child });
  }, [child, route.key]);
  if (!props || !child) {
    return null;
  }
  ...
  const onUpdate: HvComponentOnUpdate = (
    ...
    if (action === 'swap' && opts.newElement) {
      setElements({ ...elements, [route.key]: opts.newElement });
    }
  };
  // console.warn(route);
