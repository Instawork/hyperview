...
import type { Validator, ValidatorRegistry } from 'hyperview/src/types';
import MaxLengthValidator from 'hyperview/src/validators/max-length';
import MaxValueValidator from 'hyperview/src/validators/max-value';
import MinLengthValidator from 'hyperview/src/validators/min-length';
import MinValueValidator from 'hyperview/src/validators/min-value';
...
export const getValidators = (
  ...
  return elementChildren.reduce(
    (validators: Array<[Validator, Element]>, e: Element) => {
      ...
      const validator: Validator | null = e.localName
        ? namespace[e.localName]
        : null;
      if (validator) {
...
export const getValidatorElementsWithInvalidState = (
  ...
  return (
    getValidators(element)
      ...
      .filter((e: Element) => {
        return (
          e.getAttributeNS(Namespaces.HYPERVIEW_VALIDATION, 'state') ===
          'invalid'
        );
      })
  );
...
export const getValidationState = (element: Element): string | null => {
  const state: string | null = element.getAttributeNS(
  ...
  if (sourceId !== null && sourceId !== undefined) {
    ...
    const sourceElement: Element | null = doc
      ? doc.getElementById(sourceId)
      : null;
    return sourceElement
      ? sourceElement.getAttributeNS(Namespaces.HYPERVIEW_VALIDATION, 'state')
      : null;
  }
...
export const getValidationRoles = (element: Element): Array<string> => {
  const role: string =
    element.getAttributeNS(Namespaces.HYPERVIEW_VALIDATION, 'role') || '';
